...
void IssueTrackingSystem::showAllIssues() const{
    ...
    for(int i = 0; i< numOfEmployees; i++){
        ...
        string currentName = current.getEmployeeName();
        for (int j = 0; j < numOfIssues; j++){
            if (issues[j].getAssignee().getEmployeeName()==currentName){
                Issue temp = issues[j];
                cout<<temp.getIssueId()<< ", " << temp.getDescription() << ", " << temp.getAssignee().getEmployeeName() << "." <<endl;
            }
        }
    }
}
